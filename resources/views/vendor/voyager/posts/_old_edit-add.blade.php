            ...
            <div class="col-md-8">
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        @php
                        $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                        $exclude = ['title', 'body', 'excerpt', 'slug', 'type', 'status', 'category_id', 'author_id', 'featured', 'image', 'meta_description', 'meta_keywords', 'seo_title'];
                        @endphp
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="type" id="type_id">
                                <option value="articles" @if(isset($dataTypeContent->type) && $dataTypeContent->type == "articles") selected="selected"@endif>Articles</option>
                                <option value="caseStudy" @if(isset($dataTypeContent->type) && $dataTypeContent->type == "caseStudy") selected="selected"@endif>Case Study</option>
                            </select>
                        </div>
                        <div class="form-group" id="articles_id_parent" <?php
                        if (isset($dataTypeContent->type) && $dataTypeContent->type == "caseStudy") {
                            echo "style='display:none'";
                        }
                        ?>">
                            <label for="category_id">{{ __('voyager::post.category') }}</label>
                            <select class="form-control" name="<?php
                            if (isset($dataTypeContent->type) && $dataTypeContent->type == "") {
                                echo "category_id";
                            } else {
                                echo "category_id_old";
                            }
                            ?>" id="articles_id">
                                @foreach(Voyager::model('Category')::where('type','articles')->get() as $category)
                                <option value="{{ $category->id }}"@if(isset($dataTypeContent->category_id) && $dataTypeContent->category_id == $category->id) selected="selected"@endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group" id="caseStudy_id_parent" <?php
                        if (isset($dataTypeContent->type) && $dataTypeContent->type == "articles") {
                            echo "style='display:none'";
                        }
                        ?>">
                            <label for="category_id">{{ __('voyager::post.category') }}</label>
                            ...
                            ?>" id="caseStudy_id">>
                                @foreach(Voyager::model('Category')::where('type','caseStudy')->get() as $category)
                                <option value="{{ $category->id }}"@if(isset($dataTypeContent->category_id) && $dataTypeContent->category_id == $category->id) selected="selected"@endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
